...
class ColorfulDelegate(
    ...
    @StyleRes private val customMaterialYouTheme: Int = 0
) {
    ...
    fun apply(
        ...
        if (override) {
            when (baseTheme) {
                ...
                THEME_MATERIAL_YOU -> activity.setTheme(
                    if (customMaterialYouTheme != 0) {
                        customMaterialYouTheme
                    } else {
                        if (darkTheme) R.style.Colorful_MaterialYou_Dark else R.style.Colorful_MaterialYou_Light
                    }
                )
            }
        }
        ...
        if (customTheme != 0) {
            ...
            Log.d(
                "COLORFUL",
                "Overriding native colorful theme with user provided theme at StyleRes $customTheme"
            )
        }
    }
